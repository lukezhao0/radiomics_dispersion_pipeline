...
def _metrics_cards_from_summary(metrics: Dict[str, Any]) -> Dict[str, tuple[str, str]]:
    ...
    return {
        ...
        "High/low accuracy": (_fmt_float(dhl.get("accuracy")), f"From LLM binary label; cutoff >= {int(config.DISPERSION_HIGH_THRESHOLD)}"),
        "High/low F1": (_fmt_float(dhl.get("f1")), "Balance of precision and recall"),
        "High/low AUROC": (_fmt_float(dhl.get("auroc")), "Ranking by predicted dispersion score"),
